...
def play():
    ...
    while not terminal(gameBoard):
        ...
        if current_player == human_player:
            while True:
                try:
                    ...
                    if 0 <= move <= 8:
                        row, col = move // cols, move % cols  # Convert move to row, col
                        if gameBoard[row][col] is None:  # Check if cell is empty
                            break
                        else:
        ...
        gameBoard = result(gameBoard, (row, col))  # Pass row, col as a tuple to result()
        draw_board(gameBoard)
